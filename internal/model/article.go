...
func (a Article) ListByTagID(db *gorm.DB, tagID uint32, pageOffset, pageSize int) ([]*ArticleRow, error) {
	...
	rows, err := db.Select(fields).Table(ArticleTag{}.TableName()+" as at").
		...
		Joins("LEFT JOIN `"+Article{}.TableName()+"` as ar on at.article_id=ar.id").Where("at.`tag_id` = ? and ar.state=? and ar.is_del=?", tagID, a.State, 0).Rows()
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (a Article) CountByTagID(db *gorm.DB, tagID uint32) (int, error) {
	...
	err := db.Table(ArticleTag{}.TableName()+" as at").
		Joins("LEFT JOIN `"+Tag{}.TableName()+"` as t on at.tag_id=t.id").
		Joins("LEFT JOIN `"+Article{}.TableName()+"` as ar on at.article_id=ar.id").Where("at.`tag_id` = ? and ar.state=? and ar.is_del=?", tagID, a.State, 0).Count(&count).Error
	if err != nil {
		return 0, err
	}
	return count, nil
